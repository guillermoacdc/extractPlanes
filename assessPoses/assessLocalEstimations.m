...
for k=1:length(planeTypes)
    ...
    outputFileName=['localAssessment_planeType' num2str(planeType) '.json'];
...
%                 localPlanes.values=assembleGlobalPlanes(tempData,index);
                localPlanes.values=estimatedPoses.(['frame' num2str(frameID)]).values.values(index);
                localPlanes.Nnap=estimatedPoses.(['frame' num2str(frameID)]).Nnap;            
            end
...
%             gtPlanes=loadGTPlanes(sessionID,frameID);
            ...
            disp(myLegend)
            [TPhl2, TPm, FPhl2, FNm] = computeTPFPFNByFrame_vplaneTracking(localPlanes,...
                gtPlanes, tao, theta, sessionID, NpointsDiagPpal);

%             [TPhl2, TPm, FPhl2, FNm] = computeMetricsByFrame_v3(localPlanes, sessionID, ...
%                 frameID, gtPlanes, tao, theta, NpointsDiagPpal, planeType);
            ...
            if isempty(localPlanes)
                ...
                assessment.(['frame' num2str(frameID)]).Nnap=localPlanes.Nnap;
            end
...
% this section
figure,
    plotEstimationsByFrame_v2(localPlanes.values, planeType, sessionID, frameID);%script