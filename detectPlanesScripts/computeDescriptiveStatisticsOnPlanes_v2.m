function [acceptedPlanes, discardedByDorNormal, discardedByLength, topOccludedPlanes]= computeDescriptiveStatisticsOnPlanes_v2(myPlanes)
...
for i=1:length(myPlanes)
    ...
    if(myPlanes{i}.type==2 | myPlanes{i}.DFlag==1)
        discardedByDorNormal(k1,:)=myPlanes{i}.getID;
        k1=k1+1;
%         continue;
    end
    
    if (myPlanes{i}.lengthFlag==1)
        discardedByLength(k2,:)=myPlanes{i}.getID;
        k2=k2+1;
    end

    if (myPlanes{i}.lengthFlag==0 & ( isempty(myPlanes{i}.DFlag) | myPlanes{i}.DFlag==0) )
        acceptedPlanes(k3,:)=myPlanes{i}.getID;
        k3=k3+1;
    end
    
    if (myPlanes{i}.antiparallelFlag==1)
        antiparallelPlanes(k4,:)=myPlanes{i}.getID;
        k4=k4+1;
    end

	if (myPlanes{i}.topOccludedPlaneFlag==1)
        topOccludedPlanes(k5,:)=myPlanes{i}.getID;
        k5=k5+1;
    end
