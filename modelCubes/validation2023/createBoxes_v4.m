function [myBoxes, flagFirstBox] = createBoxes_v4(myPlanes, localAssignedP, myBoxes)
...
%         extract IDs that belong to a common box in tempIDs
    ...
    thirdFrame=myPlanes.(['fr' num2str(targetFrame)]).values(targetElement).thirdPlaneID(1);
    thirdElement=myPlanes.(['fr' num2str(targetFrame)]).values(targetElement).thirdPlaneID(2);

    tempIDs=[localAssignedP(i,:);...
    myPlanes.(['fr' num2str(secondFrame)]).values(secondElement).getID;...
    myPlanes.(['fr' num2str(thirdFrame)]).values(thirdElement).getID];
% create type vector; with binary values
    typeVector=[myPlanes.(['fr' num2str(targetFrame)]).values(targetElement).type,...
    myPlanes.(['fr' num2str(secondFrame)]).values(secondElement).type,...
    myPlanes.(['fr' num2str(thirdFrame)]).values(thirdElement).type];
...
for i=1:size(allBoxesIDs_unique,1)
    mySingleBox=createBoxObject_v2(myPlanes,allBoxesIDs_unique(i,:),i);
