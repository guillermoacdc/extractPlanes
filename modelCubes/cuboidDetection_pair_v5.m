function  cuboidDetection_pair_v5(myPlanes, th_angle, ...
...
%     then add it to the searchSpaceT or searchspace
    if ~isempty(myPlanes.(['fr' num2str(targetFrame)])(targetElement).secondPlaneID)
        ...
        searchSpaceT=[searchSpaceT; myPlanes.(['fr' num2str(secondFrame)])(secondElement).getID];
    end
...
for i=1:size(secondPlaneT_unique,1)
    ...
    [thirdPlaneIndex detectionFlag]=thirdPlaneDetection_v4(myPlanes, ...
        targetP, secondP, planesIDs, tPExemplar{i}.crossPd, th_angle, modelTree);
%     disp()
