...
frame=2;
in_planesFolderPath=['C:/lib/outputPlanes_t4/frame ('  num2str(frame)  ')/'];%extracted planes with efficientRANSAC
...
% for i=1:47:48
    ...
    [modelParameters pc_j ]=loadPlaneModel_v2(in_planesFolderPath, frame, i);
        ...
        if(i==1)
            groundD=modelParameters(4);%update ground D. Must be loaded from algortihmParameters.txt
        end
        [x y z flag] = createPlaneFromModel_v2(xp, yp, zp, modelParameters(1), ...
            modelParameters(2), modelParameters(3), modelParameters(4), groundD);
...
%             view(108,35)
            title (['Plane Number: ' num2str(i) ' with ' num2str(pc_j.Count) ' Points from ' num2str(pc_raw.Count) '. Doffset=' num2str(abs(groundD-modelParameters(4))) ])
            pause()
            F(k) = getframe(gcf) ;
            k=k+1;
            drawnow
...
% create the video writer with 1 fps
  writerObj = VideoWriter('otherPlanes.avi');
