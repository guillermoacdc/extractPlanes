...
#include <ransac/PointCloud.h>
#include <ransac/RansacShapeDetector.h>
#include <ransac/PlanePrimitiveShapeConstructor.h>
#include <ransac/CylinderPrimitiveShapeConstructor.h>
#include <ransac/SpherePrimitiveShapeConstructor.h>
#include <ransac/ConePrimitiveShapeConstructor.h>
#include <ransac/TorusPrimitiveShapeConstructor.h>
...
int main()
{
	PointCloud pc;
	int points=100;
	int points_half=points/2;
	// fill or load point cloud from file
	for(int i=0;i<points;i++){
		for(int j=0;j<points;j++){
			pc.push_back(Point(Vec3f(i,j,0)));
			if(fabs((i-points_half)*(i-points_half)+(j-points_half)*(j-points_half)-points_half)<5){
				//std::cout << i << "," << j << "\n";
	...
	// set the bbox in pc
	pc.setBBox(Vec3f(-points_half,-points_half,-points_half), Vec3f(points_half,points_half,points_half));
