function myBoxTracker(sessionID, outputFileName, planeFilteringParameters, ...
...
% update globalPlanesPrevious vector--------------        
        ...
        estimatedPlanesPose.(['frame' num2str(frameID)]).values=obj2struct_vector(globalPlanes); 
        estimatedBoxPose.(['frame' num2str(frameID)]).values=obj2struct_vector(globalBoxes); 
        %     additional properties of estimatedBoxPose (Nnap, processingTime)
...
% estimatedPose_s=struct(estimatedBoxPose);
mySaveStruct2JSONFile(estimatedBoxPose,outputFileName,evalPath,sessionID);
...
fc='b';
figure,
    myPlotBoxContour_v2(estimatedBoxPose.frame300.values,sessionID,frameID,fc)
